/*Block Device Driver To create a 512Kb Block on RAM named mydisk and Partition 
 *it into 3 Logical and 3 Extended Partition 
 * Author Sahil Chandna, Assignment-2, EEE G 547 Bits Pilani*/ 
...
{
	int ret=0;
	printk(KERN_INFO "mydordrive : open \n");
	//goto out;
...
static void my_release(struct gendisk *disk, fmode_t mode)
{
	...
	printk(KERN_INFO "mydordrive : closed \n");
...
static int rb_transfer(struct request *req)
{
	...
	rq_for_each_segment(bv, req, iter)
	{
		...
		printk(KERN_DEBUG "mydor: Start Sector: %llu, Sector Offset: %llu;\
	...
	{
		printk(KERN_ERR "mydor: bio info doesn't match with the request info");
		ret = -EIO;
	}
...
{
	mydisk_init();
	c = register_blkdev(c, "mydor");// major no. allocation
	...
	sprintf(((device.gd)->disk_name), "mydor");
...
{	
	...
	unregister_blkdev(c, "mydor");
	put_disk(device.gd);
	
	blk_cleanup_queue(device.queue);
	mydisk_cleanup();
	spin_unlock(&device.lock);	
	printk(KERN_ALERT "mydordrive is unregistered");
